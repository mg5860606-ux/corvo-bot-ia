...
comandos.push(obj); 
fs.writeFileSync('./DADOS DO CORVO/INFO_CORVO/media/comandos.json', JSON.stringify(comandos))};
...
if (position !== false) { 
comandos[position].comandos.push(oq); fs.writeFileSync('./DADOS DO CORVO/INFO_CORVO/media/comandos.json', JSON.stringify(comandos))}
}
const deleteComandos = (from, oq) => { let position = false; Object.keys(comandos).forEach((i) => { if (comandos[i].groupId === from ){ position = i }}); if (position !== false) { comandos[position].comandos.splice(oq, 1); fs.writeFileSync('./DADOS DO CORVO/INFO_CORVO/media/comandos.json', JSON.stringify(comandos))}}
