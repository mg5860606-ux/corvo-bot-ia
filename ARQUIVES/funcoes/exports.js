...
// Funções:
const { sendVideoAsSticker, sendImageAsSticker } = require('../../ARQUIVES/sticker/rename.js');
const { sendVideoAsSticker2, sendImageAsSticker2 } = require('../../ARQUIVES/sticker/rename2.js');
const { awaitMessage } = require('../../ARQUIVES/funcoes/awaitMessage.js');
...
var EnvBotao = async (id, MR, corvo, text1, desc1, X) => {
if(X.split("|")[1] != "0") {
corvo.sendMessage(id, {image: {url: X.split("|")[1]}, caption: text1, mentions: [MR]})} else {corvo.sendMessage(id, {text: text1, mentions: [MR]})}}
} else {
var EnvBotao = async(id, MR, corvo, text1, desc1, X, TXT = [], vr) => {
...
const listCommands = (targetWord) => {
      const fileContent = fs.readFileSync("corvo.js", "utf8");
