...
const filePath = 'c:\\corvo-bot-corrigido\\corvo.js';
...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes("body: { text: '*Resultados da metadinha* 💝 ↴' },")) {
        startLine = i + 1;
    }
    if (lines[i].includes("site(quote, 40, 40)")) {
        endLine = i;
        break;
    }
}
...
if (startLine !== -1 && endLine !== -1) {
    console.log(`Fixing lines ${startLine + 1} to ${endLine + 1}`);
    ...
    console.log(`Could not find anchors: startLine=${startLine}, endLine=${endLine}`);
}
